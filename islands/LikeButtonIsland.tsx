...
import Icon from "deco-sites/brunightcampv2/components/ui/Icon.tsx";
import { invoke } from "deco-sites/brunightcampv2/runtime.ts";
import { total } from "deco-sites/brunightcampv2/sdk/useTotalLikes.ts";
...
function LikeButtonIsland({ productID }: LikeButtonIslandProps) {
  ...
  useEffect(() => {
    const updateTotals = async () => {
      const totalLikes = await invoke["deco-sites/brunightcampv2"].loaders
        .totalLikesLoader();
      const totalLikesProduct = await invoke["deco-sites/brunightcampv2"].loaders
  ...
  const handleToggleLike = async (e: MouseEvent) => {
    e.preventDefault();
    const result = await invoke["deco-sites/brunightcampv2"].actions
